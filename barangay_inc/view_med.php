    ...
    <div class="row">
        <?php if (!empty($records) && is_array($records)): ?>
            <?php foreach ($records as $row): ?>
                    ...
                    <div class="card product-card shadow-sm">
                        ...
                        <div class="product-info">
                            ...
                            <button class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                data-bs-target="#medModal<?php echo $row['id']; ?>">View Details</button>
                            <a href="request_med.php?city_health_id=<?= htmlspecialchars($row['city_health_id']) ?>&id=<?= htmlspecialchars($row['id']) ?>"
                ...
                <!-- Modal -->
                <div class="modal fade" id="medModal<?php echo $row['id']; ?>" tabindex="-1"
                    aria-labelledby="medModalLabel<?php echo $row['id']; ?>" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p class="product-description"><strong>Description:</strong>
                                    <?php echo htmlspecialchars($row['med_description'] ?? ''); ?></p>
                                <p class="product-description"><strong>Category:</strong>
                                    <?php echo htmlspecialchars($row['category'] ?? ''); ?></p>
                                <p class="product-description"><strong>Dosage Form:</strong>
                                    <?php echo htmlspecialchars($row['DosageForm'] ?? ''); ?></p>
                                <p class="product-description"><strong>Dosage Strength:</strong>
                                    <?php echo htmlspecialchars($row['DosageStrength'] ?? ''); ?></p>



                                <p class="product-quantity"><strong>Quantity:</strong>
                                    <?php echo htmlspecialchars($row['quantity'] ?? ''); ?></p>
                                <p class="product-date"><strong>Expiry:</strong>
                                    <?php echo !empty($row['expiry_date']) ? date('F d, Y', strtotime($row['expiry_date'])) : 'N/A'; ?>
                                </p>
                            </div>
